...
public class OrGateOffNeighbourBlockChangesProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, BlockState blockstate) {
		...
		if (power) {
			if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == VanillaPlusModBlocks.OR_GATE_OFF.get()) {
				VanillaPlusMod.queueServerWork(1, () -> {
					{
						BlockPos _bp = BlockPos.containing(x, y, z);
						BlockState _bs = VanillaPlusModBlocks.OR_GATE_ON.get().defaultBlockState();
			...
			if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == VanillaPlusModBlocks.OR_GATE_ON.get()) {
				VanillaPlusMod.queueServerWork(1, () -> {
					{
						BlockPos _bp = BlockPos.containing(x, y, z);
						BlockState _bs = VanillaPlusModBlocks.OR_GATE_OFF.get().defaultBlockState();
						BlockState _bso = world.getBlockState(_bp);
						for (Property<?> _propertyOld : _bso.getProperties()) {
							Property _propertyNew = _bs.getBlock().getStateDefinition().getProperty(_propertyOld.getName());
							if (_propertyNew != null && _bs.getValue(_propertyNew) != null)
